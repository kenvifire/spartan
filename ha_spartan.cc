...
uchar *ha_spartan::get_key()
{
    ...
    for (Field **field=table->field; *field; field++) {
        if ((*field)->key_start.to_ulonglong() == 1) {
            key = (uchar*)my_malloc(PSI_INSTRUMENT_ME,(*field)->field_length,
...
int ha_spartan::open(const char *name, int mode, uint test_if_locked)
{
    ...
    share->data_class->open_table(fn_format(name_buf, name_buf, "", SDE_EXT,
                                            MY_REPLACE_EXT | MY_UNPACK_FILENAME));
    share->index_class->open_index(fn_format(name_buf, name, "", SDI_EXT,
                                             MY_REPLACE_EXT |
                                             MY_UNPACK_FILENAME));
...
int ha_spartan::update_row(const uchar *old_data, uchar *new_data)
{
    ...
    if (get_key() != 0) {
        share->index_class->update_key(get_key(),
...
int ha_spartan::delete_row(const uchar *buf)
{
    ...
    share->data_class->delete_row((uchar*)buf, table->s->rec_buff_length,
                                  share->data_class->row_size(table->s->rec_buff_length));
...
int ha_spartan::delete_table(const char *name)
{
    ...
    my_delete(fn_format(name_buf, name, "", SDE_EXT,
                        MY_REPLACE_EXT | MY_UNPACK_FILENAME), MYF(0));
    my_delete(fn_format(name_buf, name, "", SDI_EXT,
                        MY_REPLACE_EXT | MY_UNPACK_FILENAME), MYF(0));
    DBUG_RETURN(0);
}
...
int ha_spartan::rename_table(const char * from, const char * to)
{
    ...
    my_rename(fn_format(index_from, from, "", SDI_EXT,
                        MY_REPLACE_EXT | MY_UNPACK_FILENAME),
                       ...
                       HA_CREATE_INFO *create_info)
{
    ...
    if (share->data_class->create_table(fn_format(name_buf, name, "", SDE_EXT, MY_REPLACE_EXT
                                                  |MY_UNPACK_FILENAME))) {
        DBUG_RETURN(-1);
    }
    share->data_class->close_table();
...
spartan_vars_t spartan_vars= {100, 20.01, "three hundred", true, 0, 8250};

static st_mysql_show_var show_status_spartan[]=
{
...
static struct st_mysql_show_var show_array_spartan[]=
{
...
{
    {"spartan_func_spartan", (char *)show_func_spartan, SHOW_FUNC, SHOW_SCOPE_GLOBAL},
    {"spartan_status_var5", (char *)&spartan_vars.var5, SHOW_BOOL, SHOW_SCOPE_GLOBAL},
    {"spartan_status_var6", (char *)&spartan_vars.var6, SHOW_LONG, SHOW_SCOPE_GLOBAL},
    {"spartan_status",  (char *)show_array_spartan, SHOW_ARRAY, SHOW_SCOPE_GLOBAL},
    {0,0,SHOW_UNDEF, SHOW_SCOPE_UNDEF}
};
